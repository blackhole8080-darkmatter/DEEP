// <audit-view> — the tamper-evident audit ledger: chain-integrity badge,
// stats, and a live entry stream from /audit/*.
import { LitElement, html, css } from "lit";
import { customElement, state } from "lit/decorators.js";
import "../primitives/ds-panel";

interface AuditStats {
  total_entries: number; entries_today: number;
  entries_by_type: Record<string, number>;
  threat_count_today: number; threat_count_total: number;
  chain_intact: boolean; last_integrity_check: string;
}
interface AuditEntry {
  id: number; timestamp: string; entry_type: string;
...
export class AuditView extends LitElement {
  ...
  private async refresh(): Promise<void> {
    void fetch("/audit/stats").then((r) => r.json()).then((d) => (this.stats = d)).catch(() => {});
    void fetch("/audit/log?limit=40").then((r) => r.json()).then((d) => (this.entries = d.entries ?? [])).catch(() => {});
  }
      ...
      ${s ? html`
        ...
        <ds-panel heading="Ledger stats">
          ...
          <div style="margin-top:var(--ds-space-3)" class="tags">
            ${Object.entries(s.entries_by_type).map(([t, n]) => html`<span class="tag">${t} ${n}</span>`)}
          </div>
        </ds-panel>
      ` : html`<span class="muted">loading ledger…</span>`}

      <ds-panel heading="Recent entries · live">
        <div class="log">
          ${this.entries.map((e) => html`
            <div class="row">
              <span class="t">#${e.id}</span>
              <span class="ty ${e.entry_type}">${e.entry_type}</span>
              <span>${e.action}</span>
            </div>
          `)}
        </div>
