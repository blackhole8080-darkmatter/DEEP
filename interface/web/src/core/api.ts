...
export interface TelemetrySummary {
  window_hours?: number;
  error?: string;
  llm?: { total_calls: number; ok: number; fallthroughs: number; p50_ms: number | null; p95_ms: number | null; est_tokens?: number; est_cost_usd?: number; providers: { provider: string; calls: number; ok: number; p50_ms: number | null; p95_ms: number | null; tokens?: number; cost?: number }[] };
