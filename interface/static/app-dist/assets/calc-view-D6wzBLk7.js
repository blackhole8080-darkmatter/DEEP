import{i as E,G as m,J as H,K as M,d as l,e as _,r as n,t as q}from"./index-Cnh7q9sv.js";import{r as k,o as z,k as Q}from"./math-Dj-7EUYl.js";import"./ds-panel-h3qMLIrJ.js";import"./ds-button-p2sgz0xK.js";var S=Object.defineProperty,P=Object.getOwnPropertyDescriptor,p=(e,r,t,o)=>{for(var i=o>1?void 0:o?P(r,t):r,c=e.length-1,u;c>=0;c--)(u=e[c])&&(i=(o?u(r,t,i):u(i))||i);return o&&i&&S(r,t,i),i};function C(e){const r=e.replace(/\^/g,"**").replace(/\b(sin|cos|tan|asin|acos|atan|sqrt|abs|log|log2|log10|exp|sign|floor|ceil|round|cbrt|sinh|cosh|tanh)\b/g,"Math.$1").replace(/\bpi\b/gi,"Math.PI").replace(/\be\b/g,"Math.E").replace(/\bln\b/g,"Math.log");if(/[^0-9x+\-*/().,%\sMathPIE_a-z]/i.test(r))return null;try{const t=new Function("x",`"use strict"; return (${r});`);return typeof t(1)!="number"||Number.isNaN(t(1))&&Number.isNaN(t(.5)),t}catch{return null}}let a=class extends E{constructor(){super(...arguments),this.expr="sin(x) * x",this.xmin=-10,this.xmax=10,this.error="",this.computeQ="derivative of x^3 + 2x",this.computeOut="",this.computeTeX="",this.computeTeXExpr="",this.computing=!1}plot(){const t=C(this.expr);if(!t)return this.error="invalid expression",m``;const o=this.xmin,i=this.xmax,c=480,u=[],$=[];for(let s=0;s<=c;s++){const d=o+s/c*(i-o);let N=NaN;try{N=t(d)}catch{}u.push(d),$.push(N)}const y=$.filter(s=>Number.isFinite(s));if(!y.length)return this.error="no finite values in range",m``;this.error="";let x=Math.min(...y),v=Math.max(...y);x===v&&(x-=1,v+=1);const T=(v-x)*.08;x-=T,v+=T;const h=s=>(s-o)/(i-o)*720,f=s=>380-(s-x)/(v-x)*380;let X="",w=!1;for(let s=0;s<=c;s++){const d=$[s];if(!Number.isFinite(d)||f(d)<-380||f(d)>2*380){w=!1;continue}X+=`${w?"L":"M"}${h(u[s]).toFixed(1)},${f(d).toFixed(1)} `,w=!0}const g=h(0),b=f(0),O=[];for(let s=Math.ceil(o);s<=i;s++)O.push(m`<line class="grid" x1=${h(s)} y1="0" x2=${h(s)} y2=${380}></line>`);return m`
      ...
      <ds-panel heading="Symbolic & science compute · live engine">
        <div class="compute">
          ...
          ${this.computeTeX?l`<div class="tex-block">
                ${this.computeTeXExpr?l`<div class="tex-expr">${z(this.computeTeXExpr)}</div>`:""}
                ${z(this.computeTeX)}
              </div>`:""}
...
//# sourceMappingURL=calc-view-D6wzBLk7.js.map
