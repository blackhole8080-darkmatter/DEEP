import{i as u,g as c,d as o,e as v,r as p,h,t as g}from"./index-CNV0UZxn.js";import"./ds-panel-C-QBPNy4.js";import"./ds-button-CvDvh5Ru.js";var b=Object.defineProperty,f=Object.getOwnPropertyDescriptor,d=(s,a,r,n)=>{for(var t=n>1?void 0:n?f(a,r):a,i=s.length-1,l;i>=0;i--)(l=s[i])&&(t=(n?l(a,r,t):l(t))||t);return n&&t&&b(a,r,t),t};const x=["researcher","writer","planner","analyst","coder"];let e=class extends u{constructor(){super(...arguments),this.tasks=[],this.agentRole="planner"}connectedCallback(){super.connectedCallback(),this.poll(),this.timer=setInterval(()=>void this.poll(),4e3)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this.timer)}async poll(){try{const s=await(await fetch("/api/agents/tasks")).json();this.tasks=s.tasks??[]}catch{}}async launch(){const s=this.ta.value.trim();if(s)try{const a=await fetch("/api/agents/task",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({role:this.agentRole,task:s})}),r=await a.json();a.ok?(c(`Agent launched (${this.agentRole})`,"success"),this.ta.value="",this.poll()):c(`Launch failed: ${r.detail??a.status}`,"danger")}catch{c("Launch failed","danger")}}render(){return o`
      <ds-panel heading="Launch agent mission">
        <div class="launch">
          <div class="roles">
            ${x.map(s=>o`
                <button class="chip ${this.agentRole===s?"on":""}" @click=${()=>this.agentRole=s}>${s}</button>
              `)}
          </div>
  ...
  `;d([p()],e.prototype,"tasks",2);d([p()],e.prototype,"agentRole",2);d([h("textarea")],e.prototype,"ta",2);e=d([g("agents-view")],e);export{e as AgentsView};
//# sourceMappingURL=agents-view-WHAHq5r0.js.map
