...
def _db() -> sqlite3.Connection:
    ...
    if _conn is None:
        ...
        _conn.execute(
            ...
            "fallthroughs INTEGER, note TEXT)"
        )
        _conn.execute(
...
def record_llm(provider: str, model: str, latency_ms: Optional[float] = None,
               ok: bool = True, fallthroughs: int = 0, note: str = "") -> None:
    try:
        with _lock:
            _db().execute(
                "INSERT INTO llm_calls VALUES (?,?,?,?,?,?,?)",
                (time.time(), provider, model, latency_ms, int(ok), fallthroughs, note),
            )
            _db().commit()
    except Exception:
...
def summary(hours: int = 24) -> Dict[str, Any]:
    ...
    try:
        with _lock:
            db = _db()
            llm = db.execute(
                "SELECT provider, model, latency_ms, ok, fallthroughs FROM llm_calls WHERE ts >= ?",
                (cutoff,),
            ).fetchall()
            tools = db.execute(
    ...
    for provider, model, latency, ok, ft in llm:
        key = f"{provider}:{model}" if model else provider
        p = providers.setdefault(key, {"calls": 0, "ok": 0, "lat": []})
        p["calls"] += 1
        p["ok"] += int(ok or 0)
